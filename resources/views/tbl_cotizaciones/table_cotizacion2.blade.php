            ...
            <td>
              <p style="width: 100px">
                  @if(empty($cotizados->cantidad))
                         <input style="background-color:whitesmoke" type="text"  id="cantidad{{ $cotizados->id_dt }}" value="0" class="form-control decimal-inputmask" onchange="guarda_detalles({{ $cotizados->id_dt }},{{ $num_cot }})">

                  @else
                        <input style="background-color:whitesmoke" type="text"  id="cantidad{{ $cotizados->id_dt }}" value="{{ $cotizados->cantidad }}" class="form-control decimal-inputmask" onchange="guarda_detalles({{ $cotizados->id_dt }},{{ $num_cot }})">
                  @endif
                </p>
            </td>
            <td>
            ...
            <td>
              <p style="width: 100px">
                    @if(empty($cotizados->descuento))
                          <input type="text" style="background-color:whitesmoke;text-align: right"  value="00.00" id="descuentos{{ $cotizados->id_dt }}" class="form-control percentage-mask" onchange="guarda_detalles({{ $cotizados->id_dt }},{{ $num_cot }})">

                    @else
                          <input type="text" style="background-color:whitesmoke;text-align: right" value="{{ $cotizados->descuento }}"  id="descuentos{{ $cotizados->id_dt }}" class="form-control percentage-mask" onchange="guarda_detalles({{ $cotizados->id_dt }},{{ $num_cot }})">
          ...
          @php($anticipo= $con_iva*($actual->anticipo/100))
           ...
           <tr style="width: 100%" class="gris_barra" style="color: gray" >
                ...
                <td  style="width: 130px;background-color: white;">
                   <div class="col-md-6" style="text-align: right;"> 
                        <div class="form-group row" style="width: 100px;">
                         <select style="text-align: right;text-align-last: right;width: 100%" class="form-control" id="descuento_aplicado" name="descuento_aplicado" onchange="actualiza2({{ $num_cot }})">
                          @if($actual->descuento=='00.00')
                                <option style="text-align: right;text-align-last: right;" value="0" {{ ($actual->descuento_aplicado==0) ? 'selected' : '' }}>00.00%</option>

                          @else
                                <option style="text-align: right;text-align-last: right;" value="0" {{ ($actual->descuento_aplicado==0) ? 'selected' : '' }}>00.00%</option>
                                <option style="text-align: right;text-align-last: right;" value="{{ $actual->descuento }}" {{ ($actual->descuento==$actual->descuento_aplicado) ? 'selected' : ''  }} >{{ $actual->descuento }}%</option>
            ...
            <tr style="width: 100%"  style="color: gray; background-color: white" >
                ...
                <td colspan="3" style="text-align: right;background-color: white"><textarea  style="height: 200px" class="form-control" onchange="actualiza2({{ $num_cot }})" id="comentarios_encot">{{ $actual->comentarios }}</textarea></td> 
                ...
                <td  style="width: 130px;background-color: white">
                   <div class="col-md-6"> 
                        <div class="form-group row" style="width: 100px;">
                         <select style="text-align: right;text-align-last: right;width: 100%" class="form-control" id="iva" name="iva" onchange="actualiza2({{ $num_cot }})">
                           <option style="text-align: right;text-align-last: right;" value="0"  {{ ($actual->iva==0) ? 'selected' : '' }}>00.00%</option>
                           <option style="text-align: right;text-align-last: right;" value="8"  {{ ($actual->iva==8) ? 'selected' : '' }}>08.00%</option>
                           <option style="text-align: right;text-align-last: right;" value="16" {{ ($actual->iva==16) ? 'selected' : '' }}>16.00%</option>
                         </select> 
                        </div>
                  </div> 
                </td>
             ...
             <tr style="width: 100%"  style="color: gray;" >
                ...
                <td colspan="3" style="text-align: right;background-color: white"><textarea  style="height: 200px" class="form-control" onchange="actualiza2({{ $num_cot }})" id="condiciones_encot">{{ $actual->condiciones }}</textarea></td> 
              ...
              <tr style="width: 100%"  style="color: gray; background-color: white" >
                ...
                <td colspan="3" style="background-color: white;text-align: "> 
                    @if(empty($actual->flete))
                       <input type="text" name="flete" style="background-color:whitesmoke;width: 100px;" id="flete" class="form-control currency" value="0" onchange="actualiza2({{ $num_cot }})"> 
                    @else
                        <input type="text" name="flete" style="background-color:whitesmoke;width: 100px;" id="flete" class="form-control currency" value="{{ $actual->flete }}" onchange="actualiza2({{ $num_cot }})"> 
                    @endif
                </td>
                   ...
                   <div class="col-md-6"> 
                        <div class="form-group row" style="width: 100px;">
                          @if(empty($actual->anticipo))
                           <input type="text" style="background-color:whitesmoke;text-align: left;text-align: right;text-align-last: right;"   name="anticipo" id="anticipo" class="form-control percentage-mask" value="00.00" onchange="actualiza2({{ $num_cot }})">

                          @else
                             <input type="text" style="background-color:whitesmoke;text-align: left;text-align: right;text-align-last: right;" name="anticipo" id="anticipo" class="form-control percentage-mask" value="{{ $actual->anticipo }}" onchange="actualiza2({{ $num_cot }})">
                          @endif
                        </div>
                  </div> 
                </td>
